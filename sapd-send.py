...
class FormWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.string_entry = QComboBox()  # Cambiado a QComboBox
        self.string_entry.setFont(font)
        self.string_entry.currentIndexChanged.connect(self.update_emails)
        ...
        self.generate_csv_button = QPushButton('Generar Archivo SAPD')
    ...
    def load_auth_data(self):
        self.auth_data = {}
        try:
            tree = ET.parse('data/auth.xml')
            root = tree.getroot()
            for auth in root.findall('auth'):
                centro = auth.find('centro').text
                email_principal = auth.find('email_principal').text
                email_secundario = auth.find('email_secundario').text
                self.auth_data[centro] = (email_principal, email_secundario)
                self.string_entry.addItem(centro)
        except Exception as e:
            QMessageBox.critical(self, 'Error', f'Error al leer el archivo XML: {str(e)}')

    def update_emails(self):
        centro = self.string_entry.currentText()
        if centro in self.auth_data:
            email_principal, email_secundario = self.auth_data[centro]
            self.email1_entry.setText(email_principal)
            self.email2_entry.setText(email_secundario)

    def generate_csv(self):
        # Obtener valores del formulario
        email1 = self.email1_entry.text()
        email2 = self.email2_entry.text()
        string = self.string_entry.currentText()
        ...
        QMessageBox.information(self, 'Éxito', 'Archivo SAPD (.ZIP) generado.')
        self.compra()
